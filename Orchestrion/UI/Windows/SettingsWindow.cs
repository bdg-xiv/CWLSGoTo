...
public class SettingsWindow : Window
{
    ...
    public override void Draw()
	{
        ...
        using (OrchestrionPlugin.LargeFont.Push())
        {
            ImGui.Text(Loc.Localize("LocalMusicSettings", "Local Music"));
        }

        ImGui.TextWrapped(Loc.Localize("LocalMusicHint",
            "Audio files from this folder (subfolders included) join the song list and can be mixed with " +
            "game songs in playlists. Supported: mp3, ogg, wav, m4a, aac, wma, flac."));

        var localFolder = Configuration.Instance.LocalMusicFolder;
        ImGui.SetNextItemWidth(400f * ImGuiHelpers.GlobalScale);
        if (ImGui.InputText("##orch_localmusicfolder", ref localFolder, 512))
        {
            Configuration.Instance.LocalMusicFolder = localFolder;
            Configuration.Instance.Save();
        }
        ImGui.SameLine();
        ImGui.BeginDisabled(LocalMusicManager.Scanning);
        if (ImGui.Button(LocalMusicManager.Scanning
                ? Loc.Localize("LocalMusicScanning", "Scanning...")
                : Loc.Localize("LocalMusicScan", "Scan")))
