...
public class SongList
{
    ...
    /// <summary>Replaces all local-music entries (ids >= LocalMusicManager.IdBase) with the given set.</summary>
    public void SetLocalSongs(IEnumerable<Song> songs)
    {
        foreach (var id in _songs.Keys.Where(Audio.LocalMusicManager.IsLocalId).ToList())
            _songs.Remove(id);
    ...
    public bool TryGetRandomSong(string limitToPlaylist, out int songId)
    {
        ...
        if (!source.Any(x => _songs.TryGetValue(x, out var s) && s.FileExists)) return false;
